...
export default function PurchasesPage() {
  ...
  const fetchPurchases = async () => {
    ...
    setLoadingData(true);
    try {
      const response = await fetch(`/api/purchases?userId=${user.id}&page=${page}&limit=10`);
              ...
              {loadingData ? (
                ...
                purchases.map((purchase) => (
                  <tr key={purchase.id} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium">{purchase.bill_number}</td>
                    <td className="px-6 py-4">{purchase.trader?.name || 'N/A'}</td>
                    <td className="px-6 py-4">{new Date(purchase.purchase_date).toLocaleDateString()}</td>
                    <td className="px-6 py-4">{formatCurrency(purchase.net_amount)}</td>
